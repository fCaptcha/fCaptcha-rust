...
pub async fn generate_headers(capi_version: &str, api_url: &str, headers_in: &BrowserHeaders) -> FCaptchaResult<HeaderMap> {
...
pub async fn generate_headers_capi() -> FCaptchaResult<HeaderMap> {
    let mut headers = HeaderMap::new();
    headers.insert("Accept", HeaderValue::from_str("*/*")?);
